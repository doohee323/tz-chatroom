...
public class StringUtil {
  ...
  /**
   * <pre>
   * String array 정보를 gubun 자를 포함하고  하나의 문자열로 리턴한다.
   * </pre>
   * 
   * @param values
   * @param gubun
   ...
   * @return
   */
  public static String arrayToStringFlg(String[] values, String gubun, String flag) {
    StringBuffer sb = new StringBuffer();
    if (values == null || values.length < 1)
      return "";
    sb.append(flag).append(values[0]).append(flag);
    for (int i = 1; i < values.length; i++) {
      sb.append(gubun).append(flag).append(values[i]).append(flag);
    }
    return sb.toString();
  }
   ...
   *          문장 전체의 길이
   * @param isFill
   *          변환된 문장이 지정된 길이보다 짧은 경우 여백처리 여부
